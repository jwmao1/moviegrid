...
try:
    ...
    from common import is_main_process, VIDEO_EXTENSIONS, round_to_nearest_multiple, round_down_to_multiple
import comfy.utils
import comfy.sd
...
def make_contiguous(*tensors):
    return tuple(x.contiguous() if x is not None else None for x in tensors)
...
class BasePipeline:
    ...
    def get_num_text_encoders(self):
        return len(self.get_text_encoders())
    ...
    def load_adapter_weights(self, adapter_path):
        ...
        for k, v in adapter_state_dict.items():
            ...
            if k in model_parameters:
                ...
                continue
            # Replace weight at end for LoRA format. Some adapters also store
            # ordinary trainable parameters, so only use this fallback when the
            # raw parameter name is not present.
            lora_k = re.sub(r'\.weight$', '.default.weight', k)
            if lora_k not in model_parameters:
                raise RuntimeError(f'modified_state_dict key {lora_k} is not in the model parameters')
            modified_state_dict[lora_k] = v
