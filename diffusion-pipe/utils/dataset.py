...
UNCOND_FRACTION = 0.0
PRECOMPUTED_WAN22_EMBEDDINGS = 'wan22_ti2v_embeddings'
...
def _recursive_clone_tensors(obj):
    if torch.is_tensor(obj):
        return obj.clone()
    elif isinstance(obj, dict):
        for k, v in obj.items():
            obj[k] = _recursive_clone_tensors(v)
        return obj
    elif isinstance(obj, (list, tuple)):
        return [_recursive_clone_tensors(x) for x in obj]
    else:
        return obj
...
def _resolve_precomputed_embedding_file(file_path, dataset_root):
    path = Path(file_path)
    ...
    for i in range(len(parts) - 1):
        if parts[i] == 'dataset' and parts[i + 1] == 'cache':
            tail_parts = parts[i + 2:]
            if tail_parts:
                candidate_tails.append(Path(*tail_parts))
    if len(parts) >= 2:
        candidate_tails.append(Path(*parts[-2:]))
...
def _unbatch_iter(batch):
    length = len(next(iter(batch.values())))
    for i in range(length):
        result = {}
        for key in batch:
            result[key] = batch[key][i]
        yield _recursive_clone_tensors(result)
...
def _map_and_cache(dataset, map_fn, cache_dir, cache_file_prefix='', new_fingerprint_args=None, regenerate_cache=False, caching_batch_size=1):
    ...
    for batch in tqdm(map_iter, initial=completed_batches, total=total_batches):
        for example in _unbatch_iter(batch):
            cache.add(example)
...
def _stream_map_and_cache(dataset, map_fn, cache_dir, cache_file_prefix='', new_fingerprint_args=None, regenerate_cache=False, caching_batch_size=1):
    new_fingerprint_args = [] if new_fingerprint_args is None else new_fingerprint_args
    new_fingerprint_args.append(dataset._fingerprint)
    new_fingerprint = Hasher.hash(new_fingerprint_args)
    if cache_file_prefix:
    ...
    for batch in tqdm(dataset.iter(batch_size=caching_batch_size), initial=completed_batches, total=total_batches):
        mapped_batch = map_fn(batch, 0)
        for example in _unbatch_iter(mapped_batch):
            cache.add(example)
...
class SizeBucketDataset:
    ...
    def cache_latents(self, map_fn, regenerate_cache=False, trust_cache=False, caching_batch_size=1):
        ...
        if 'video_emb_file' in self.metadata_dataset.column_names:
            def direct_latents_map_fn(example, rank):
                results = {'latents': [], 'mask': [], 'image_spec': [], 'caption': []}
                for video_emb_file, image_spec, caption in zip(
                    example['video_emb_file'], example['image_spec'], example['caption']
                ):
                    video_emb_file = _resolve_precomputed_embedding_file(video_emb_file, self.path)
                    payload = torch.load(video_emb_file, map_location='cpu')
                    latents = payload['video_latent']
                    results['latents'].append(latents)
                    results['mask'].append(None)
                    results['image_spec'].append(image_spec)
                    results['caption'].append(caption)
                return results

            self.latent_dataset = _stream_map_and_cache(
                self.metadata_dataset,
                direct_latents_map_fn,
                self.cache_dir,
            ...
            if regenerate_cache or not iteration_order_cache_dir.exists() or not trust_cache:
                iteration_order = datasets.Dataset.from_dict({
                    'image_spec': self.metadata_dataset['image_spec'],
                    'latents_idx': list(range(len(self.metadata_dataset))),
                    'caption': [''] * len(self.metadata_dataset),
                    'caption_number': [0] * len(self.metadata_dataset),
                })
    ...
    def cache_text_embeddings(self, map_fn, i, regenerate_cache=False, caching_batch_size=1):
        ...
        if 'text_emb_file' in self.metadata_dataset.column_names:
            def direct_text_map_fn(example, rank):
                results = {'text_embeddings': [], 'seq_lens': []}
                for text_emb_file in example['text_emb_file']:
                    text_emb_file = _resolve_precomputed_embedding_file(text_emb_file, self.path)
                    payload = torch.load(text_emb_file, map_location='cpu')
                    results['text_embeddings'].append(payload['text_context'])
                    results['seq_lens'].append(torch.tensor(payload['seq_len'], dtype=torch.int64))
                return results

            te_dataset = _stream_map_and_cache(
                self.metadata_dataset,
                direct_text_map_fn,
                self.cache_dir,
                cache_file_prefix=f'text_embeddings_{i}_',
            ...
            assert len(te_dataset) == len(self.metadata_dataset)
            flattened_captions = datasets.Dataset.from_dict({
                'image_spec': self.metadata_dataset['image_spec'],
                'caption': [captions[0] if len(captions) > 0 else '' for captions in self.metadata_dataset['caption']],
            })
            self.text_embedding_datasets.append(TextEmbeddingDataset(te_dataset, flattened_captions))
            return
    ...
    def __getitem__(self, idx):
        ...
        for i, ds in enumerate(self.text_embedding_datasets):
            uncond_ds = self.uncond_text_embeddings[i] if i < len(self.uncond_text_embeddings) else None
            if use_uncond and uncond_ds is not None:
    ...
    def cache_text_embeddings(self, map_fn, i, regenerate_cache=False, caching_batch_size=1):
        ...
        if 'text_emb_file' in self.metadata_dataset.column_names:
            for size_bucket_dataset in self.size_buckets:
                size_bucket_dataset.cache_text_embeddings(
                    map_fn,
                    i,
                    regenerate_cache=regenerate_cache,
                    caching_batch_size=caching_batch_size,
...
class DirectoryDataset:
    def __init__(self, directory_config, dataset_config, model_name, framerate=None, round_to_multiple=32, skip_dataset_validation=False):
        ...
        self.path = Path(self.directory_config['path'])
        self.precomputed_embeddings = self.directory_config.get('precomputed_embeddings', dataset_config.get('precomputed_embeddings', None))
    ...
    def _get_ungrouped_metadata(self, regenerate_cache=False, trust_cache=False):
        ...
        if regenerate_cache or not metadata_cache_file_1.exists() or not trust_cache:
            ...
            if self.precomputed_embeddings == PRECOMPUTED_WAN22_EMBEDDINGS:
                meta_files = list(self.path.rglob('*.meta.json'))
                meta_files.sort()
                image_specs = []
                mask_files = []
                video_emb_files = []
                ...
                for meta_file in tqdm(meta_files):
                    with open(meta_file) as f:
                        meta = json.load(f)
                    video_emb_file = meta_file.with_suffix('').with_suffix('.video_emb.pt')
                    text_emb_file = meta_file.with_suffix('').with_suffix('.text_emb.pt')
                    if not video_emb_file.exists() or not text_emb_file.exists():
                        logger.warning(f'Missing embedding pair for {meta_file}, skipping.')
                        continue
                    aligned_h, aligned_w = meta['video_stats']['aligned_hw']
                    image_specs.append((None, meta['key']))
                    mask_files.append(None)
                    video_emb_files.append(str(video_emb_file))
                    text_emb_files.append(str(text_emb_file))
                    aligned_widths.append(int(aligned_w))
                    aligned_heights.append(int(aligned_h))
                    sampled_frames.append(int(meta['video_stats']['sampled_frames']))
                    seq_lens.append(int(meta['seq_len']))
                    captions.append([''])
                assert len(image_specs) > 0, f'Directory {self.path} had no precomputed embedding pairs!'
                metadata_dataset = datasets.Dataset.from_dict({
                    'image_spec': image_specs,
                    'mask_file': mask_files,
                    'video_emb_file': video_emb_files,
                    'text_emb_file': text_emb_files,
                    'aligned_width': aligned_widths,
                    'aligned_height': aligned_heights,
    ...
    def _metadata_map_fn(self):
        ...
        def fn(example):
            ...
            if self.precomputed_embeddings == PRECOMPUTED_WAN22_EMBEDDINGS:
                ...
                width = int(example['aligned_width'][0])
                height = int(example['aligned_height'][0])
                frames = int(example['sampled_frames'][0])
                is_video = (frames > 1)
                ...
                if self.use_size_buckets:
                    ...
                    size_bucket = None
                if size_bucket is None and ar_bucket is None:
                    return empty_return
                return {
                    ...
                    'is_video': [is_video],
                    'video_emb_file': [example['video_emb_file'][0]],
                    'text_emb_file': [example['text_emb_file'][0]],
                    'seq_lens': [example['seq_len'][0]],
                }
    ...
    def cache_text_embeddings(self, map_fn, i, regenerate_cache=False, caching_batch_size=1):
        ...
        uncond_text_embeddings_ds = _stream_map_and_cache(
            empty_caption_ds,
            map_fn,
        ...
        if len(uncond_text_embeddings_ds) == 0:
            return
...
class DatasetManager:
    def __init__(self, model, regenerate_cache=False, trust_cache=False, caching_batch_size=1):
        ...
        self.submodels = None
        self.call_vae_fn = None
        self.call_text_encoder_fns = None
        self.te_fn_requires_control_file = None
        self.num_text_encoders = self.model.get_num_text_encoders()
    ...
    def _ensure_text_encoders_loaded(self):
    ...
    def _ensure_submodels_loaded(self):
        if self.submodels is not None:
            return
        self.vae = self.model.get_vae()
        self._ensure_text_encoders_loaded()
        self.submodels = [self.vae] + list(self.text_encoders)
        self.call_vae_fn = self.model.get_call_vae_fn(self.vae)

    def _unload_text_encoders(self):
        if self.text_encoders is None:
            return
        for text_encoder in self.text_encoders:
            if isinstance(text_encoder, nn.Module):
                text_encoder.to('meta')
        if hasattr(self.model, 'text_encoder') and hasattr(self.model.text_encoder, 'model'):
            self.model.text_encoder.model = None
        self.text_encoders = None
        self.call_text_encoder_fns = None
        self.te_fn_requires_control_file = None

    def _all_datasets_use_precomputed_wan22_embeddings(self):
        if len(self.datasets) == 0:
            return False
        for dataset in self.datasets:
            if len(dataset.directory_datasets) == 0:
                return False
            for directory_dataset in dataset.directory_datasets:
                if getattr(directory_dataset, 'precomputed_embeddings', None) != PRECOMPUTED_WAN22_EMBEDDINGS:
                    return False
        return True
    ...
    def cache(self, unload_models=True):
        if self.trust_cache and not self.regenerate_cache:
            if self._all_datasets_use_precomputed_wan22_embeddings():
                # Precomputed embedding datasets share on-disk metadata/latent/text caches.
                # Let rank 0 materialize/validate them first, then other ranks load read-only.
                if is_main_process():
                    for ds in self.datasets:
                        ds.cache_metadata(trust_cache=True)
                        ds.cache_latents(None, trust_cache=True)
                        for i in range(1, self.num_text_encoders + 1):
        ...
        if self._all_datasets_use_precomputed_wan22_embeddings():
            use_uncond_text_embeddings = UNCOND_FRACTION > 0 and self.num_text_encoders > 0
            if use_uncond_text_embeddings:
                self._ensure_text_encoders_loaded()
            if is_main_process():
                for ds in self.datasets:
                    ds.cache_metadata(regenerate_cache=self.regenerate_cache, trust_cache=self.trust_cache)
                for ds in self.datasets:
                    ds.cache_latents(None, regenerate_cache=self.regenerate_cache, trust_cache=self.trust_cache, caching_batch_size=self.caching_batch_size)
                for i in range(1, self.num_text_encoders + 1):
                    map_fn = None
                    if use_uncond_text_embeddings:
                        text_encoder_idx = i - 1
                        call_text_encoder_fn = self.call_text_encoder_fns[text_encoder_idx]
                        requires_control_file = self.te_fn_requires_control_file[text_encoder_idx]

                        def text_embedding_map_fn(example, rank, fn=call_text_encoder_fn, needs_control=requires_control_file):
                            control_file = example['control_file'] if 'control_file' in example else None
                        ...
                        map_fn = text_embedding_map_fn
                    for ds in self.datasets:
                        ds.cache_text_embeddings(map_fn, i, regenerate_cache=self.regenerate_cache, caching_batch_size=self.caching_batch_size)
    ...
    def _pull_batches_from_dataloader(self):
        for batch in self.dataloader:
            ...
            if not isinstance(label, (tuple, list)):
                raise TypeError(f'Expected label to be a tuple/list, got {type(label)!r}')
            if len(label) < 2:
                raise ValueError(f'Expected label to have at least target and mask, got {len(label)} items')
            target = label[0]
            mask = label[1]
            extra_label_items = tuple(label[2:])
            ...
            label = (target, mask, *extra_label_items)
