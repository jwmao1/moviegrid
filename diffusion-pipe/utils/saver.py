...
class Saver:
    ...
    def _prune_dirs(self, prefix, keep_name):
        if not is_main_process():
            return
        for path in self.save_root.glob(f'{prefix}*'):
            if not path.is_dir():
                continue
    ...
    def _prune_epoch_dirs(self, keep_name):
        if not is_main_process():
            return
        visualization_enabled = self.config.get('epoch_visualization', {}).get('enabled', False)
        for path in self.save_root.glob('epoch*'):
            if not path.is_dir():
                continue
            if path.name == keep_name:
                continue
            if visualization_enabled:
                uploaded_marker = path / '.wandb_video_uploaded'
                if not uploaded_marker.exists():
                    continue
            shutil.rmtree(path, ignore_errors=True)

    def save_visual_separator(self, save_dir):
        if not is_main_process():
            return
        transformer = getattr(self.model, 'transformer', None)
        visual_separator = getattr(transformer, 'visual_separator', None)
        if visual_separator is None:
            return
        save_path = Path(save_dir) / 'visual_separator.pt'
        tensor = visual_separator.detach().to(device='cpu')
    ...
    def save_adapter(self, name):
        ...
        if dp_id == 0 and stage_id == 0:
            ...
            self.save_visual_separator(save_dir)
    ...
    def save_model(self, name):
        ...
        if self.config.get('keep_only_latest_epoch', False) and name.startswith('epoch'):
            self._prune_epoch_dirs(name)
        dist.barrier()
    ...
    def save_checkpoint(self, step, examples):
        ...
        if self.config.get('keep_only_latest_global_checkpoint', False) and is_main_process():
            latest_file = self.save_root / 'latest'
            keep_name = latest_file.read_text().strip() if latest_file.exists() else None
            if keep_name:
                self._prune_dirs('global_step', keep_name)
        dist.barrier()
